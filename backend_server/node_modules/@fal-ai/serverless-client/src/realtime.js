...
function sendMessage(context, event) {
    if (context.websocket && context.websocket.readyState === WebSocket.OPEN) {
        if (event.message instanceof Uint8Array) {
            ...
            context.websocket.send((0, msgpackr_1.pack)(event.message));
        }
        else {
...
function buildRealtimeUrl(app, { token, maxBuffering }) {
    const { host } = (0, config_1.getConfig)();
...
function shouldSendBinary(message) {
    return Object.values(message).some((value) => value instanceof Blob ||
...
function getToken(app) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        const token = yield (0, request_1.dispatchRequest)('POST', `https://${(0, config_1.getRestApiUrl)()}/tokens/`, {
            ...
            if (machine.current === 'connecting' &&
                ...
                ws.onmessage = (event) => {
                    ...
                    if (event.data instanceof ArrayBuffer) {
                        const result = (0, msgpackr_1.unpack)(new Uint8Array(event.data));
                        onResult(result);
                        return;
                    }
                    if (event.data instanceof Uint8Array) {
                        const result = (0, msgpackr_1.unpack)(event.data);
                        onResult(result);
                        return;
                    }
                    if (event.data instanceof Blob) {
                        event.data.arrayBuffer().then((buffer) => {
                            const result = (0, msgpackr_1.unpack)(new Uint8Array(buffer));
                            onResult(result);
                        });
                        return;
                    }
